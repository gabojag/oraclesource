...
-- 4) GOREIGN KEY(외래키) : 특정 테이블의 기본키로 지정한 열을 다른 테이블의 특정 열에서 참조
-- 부모키 / 자식키
...
-- 자식 레코드가 발견되었습니다(참조하는 테이블의 지식 DELETE)
DELETE FROM DEPT_FK WHERE DEPTNO=50;



-- FOREIGN KEY
-- 1) ON DELETE CSCADE : 부모가 삭ㅈ되면 자식도 같이 삭제
-- 2) ON DELETE SET NULL : 부모가 삭제 되면 자식이 참조하는 부모의 값을 NULL 변경 
...
-- 5) CHECK : 열에 저장할 수 있는값의 범위 혹은 패턴 정의
...
-- 체크 제약조건 (SCOTT,TBLCK,PWD_CK)이 위배
...
-- DEAULT : 특정 열에 값이 지정되지 않았을 때 기본값 주기
CREATE TABLE TBL_DEFAULT(
	LOGIN_ID VARCHAR2(20) PRIMARY KEY,
	LOGIN_PWD VARCHAR2(20) CONSTRAINT TBL_DEFAULT_PED_CK CHECK 
...
CREATE TABLE COMPANY(
	...
	NAME VARCHAR(20) CONSTRAINT COM_NAME_NN NOT NULL,
	LOC VARCHAR(20) CONSTRAINT COM_LOC_NN NOT NULL,
	WORKER VARCHAR(20) CONSTRAINT COM_WORKER_NN NOT NULL
);
...
CREATE TABLE member(
	...
	JOB VARCHAR2(15),
	GRADE VARCHAR2(10) DEFAULT 'SILVER' CHECK(GRADE IN ('SILBER','GOLD', 'VIP')),
	POINT NUMBER(8) DEFAULT 0
);

INSERT INTO MEMBER(userid,password,name)
VALUES('kim123','1234','김치호');

INSERT INTO MEMBER(userid,password,name,age)
VALUES('kim123','1234','김치호',15);
...
CREATE  TABLE NENBER_ORDER( 
	ORDER_ID VALUES NUMBER PRIMARY KEY,
...
-- 제약조건
CREATE TABLE EMP_FK2( 
	EMPNO NUMBER(4) CONSTRAINT EMFK4_EMPNO_PK PRIMARY KEY,
	ENMAE VARCHAR2(10),
	JOB VARCHAR2(9),
	MGR NUMBER(4),
	HIREDATE DATE,
